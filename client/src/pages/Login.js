...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    try {
      const res = await axios.post("https://jobpilot-1z1z.onrender.com/api/auth/login", form);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center" style={{background: "linear-gradient(135deg, #667eea 0%, #764ba2 50%, #f093fb 100%)"}}>
      <div className="bg-white p-8 rounded-2xl shadow-md w-full max-w-md">
        <h1 className="text-3xl font-bold text-center text-blue-600 mb-2">JobPilot</h1>
        <p className="text-center text-gray-500 mb-6">Track your job applications</p>
